...
final class RealCall implements Call {
  ...
  final class AsyncCall extends NamedRunnable {     //this在这
  ...
  Response getResponseWithInterceptorChain() throws IOException {   //这里是Okhttp最强大的地方(个人感觉
    // Build a full stack of interceptors.
    List<Interceptor> interceptors = new ArrayList<>();
    interceptors.addAll(client.interceptors()); //这里就是建造者加入的interceptor
    ...
    Interceptor.Chain chain = new RealInterceptorChain(interceptors, transmitter, null, 0,
        ...
        client.readTimeoutMillis(), client.writeTimeoutMillis());//0就是开始的第一个

    boolean calledNoMoreExchanges = false;
    try {
      Response response = chain.proceed(originalRequest);//这里开始
